...
#   --cadence SEC      seconds between samples (default 10).
#   --timeout-ms MS    per-sample resolve timeout in ms (default 5000).
#   --duration SEC     stop after this many seconds (default: run until
#                      SIGINT/SIGTERM).
#   --conf FILE        conf to source for ZGW_HOST/ZGW_USER (used only for
#                      HomeID auto-detect). Default: ../conf next to this
#                      script's test dir.
#
# CSV columns: sample_iso,recv_iso,latency_ms,hostname,address,status
...
# timeout(1) takes seconds; allow fractional from the ms value.
TIMEOUT_S=$(awk -v ms="${TIMEOUT_MS}" 'BEGIN{printf "%.3f", ms/1000.0}')

mkdir -p "$(dirname "${OUT}")"
echo "sample_iso,recv_iso,latency_ms,hostname,address,status" > "${OUT}"
echo "ST-01 heartbeat -> ${OUT}" >&2
echo "  target=${HOSTNAME_MDNS} cadence=${CADENCE_S}s timeout=${TIMEOUT_MS}ms duration=${DURATION_S:-until-signal}" >&2
...
while true; do
  ...
  echo "${sample_iso},${recv_iso},${latency_ms},${HOSTNAME_MDNS},${address},${status}" >> "${OUT}"
  samples=$((samples+1))
